...
class Heap {
	public:
		...
		Heap(uint32 _batchNum,
            ...
            cudaMalloc((void **)&max_benefit, sizeof(int));
            cudaMemset(max_benefit, 0, sizeof(int));
#ifdef PERF_DEBUG
            cudaMalloc((void **)&maxHeapItems, sizeof(uint32));
            cudaMemset(maxHeapItems, 0, sizeof(uint32));
            cudaMalloc((void **)&totalInsNum, sizeof(uint32));
            cudaMemset(totalInsNum, 0, sizeof(uint32));
            cudaMalloc((void **)&partialNum, sizeof(uint32));
            cudaMemset(partialNum, 0, sizeof(uint32));
            cudaMalloc((void **)&fullNum, sizeof(uint32));
            cudaMemset(fullNum, 0, sizeof(uint32));
#endif
		...
		~Heap() {
            ...
            cudaFree(max_benefit);
            max_benefit = NULL;
#ifdef PERF_DEBUG
            cudaFree(maxHeapItems);
            maxHeapItems = NULL;
            cudaFree(totalInsNum);
            totalInsNum = NULL;
            cudaFree(partialNum);
            partialNum = NULL;
            cudaFree(fullNum);
        ...
        int nodeCount() {
            uint32 bcount;
            cudaMemcpy(&bcount, batchCount, sizeof(uint32), cudaMemcpyDeviceToHost);
            return (int)bcount;
        }
		...
		__device__ bool deleteRoot(uint128 *items, uint32 &size) {
			...
			if (!threadIdx.x) {
				changeStatus(&status[0], AVAIL, INUSE);
//                printf("%u -1\n", (*batchCount) * batchSize + (*partialBatchSize));
//                printf("%u %u\n", (*batchCount) * batchSize + (*partialBatchSize), *max_benefit);
#ifdef PERF_DEBUG
                atomicMax(maxHeapItems, batchSize * (*batchCount) + *partialBatchSize);
//                atomicMax(maxHeapItems, *batchCount);
#endif
			}
			__syncthreads();
...
//                printf("%u %u\n", (*batchCount) * batchSize + (*partialBatchSize), *max_benefit);
#ifdef PERF_DEBUG 
                atomicAdd(totalInsNum, size);
                if (size < batchSize) 
                    atomicAdd(partialNum, 1);
                else 
                    atomicAdd(fullNum, 1);
#endif
			}
