...
__global__ void ssspKernel(Heap_With_Aux < sssp_heap_node, int > * heap, int * edge_list_index, int * edge_dst, int * edge_weight, int * distance, int * is_empty, int block_count){
  extern __shared__ char smem[];
  int curr_smem_offset = 0;
  DECLARE_SMEM(int, p_size, 1);
  DECLARE_SMEM(int, p_total_task, 1);
  DECLARE_SMEM(int, p_should_return, 1);
  ...
  //We don't know how many nodes will be inserted
  ...
  do {
	...
	if(p_batch[i].curr_dist > distance[p_batch[i].vert]){
	  ...
	  p_rem_edge_count[i] = remaining_edge;
    ...
    SINGLE_THREADED {
      if(*p_size == 0) {
        *p_should_return = 1;
        is_empty[blockIdx.x] = 1;
	for(int i = 0;i < block_count;++i){
	  if(is_empty[i] == 0) {*p_should_return = 0;break;}
	}
      } else {
        is_empty[blockIdx.x] = 0;
	*p_should_return = 0;
      }
    }
    __syncthreads();
    if(*p_should_return) {return;} else if(*p_size == 0) {continue;}
	...
	Binary search might be faster. But will it lead to worse warp divergence?
      */
      while(curr_diff > 0){
	int remaining_edge = p_rem_edge_count[curr_vert];
