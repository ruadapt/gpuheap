...
//	HeapItem *del_items = (HeapItem *)&smem[0];
    ...
    uint32_t smemOffset = (sizeof(HeapItem) * batch_size * (EXPAND_NUMBER + 1 + 1)
                            + sizeof(uint32_t)
                            + sizeof(uint32_t) * 2) / sizeof(int);
...
#ifdef DEBUG
            if (*del_size != UINT_MAX) {
                if (atomicCAS(&opt_flag, 0, 1) == 0) {
                    printf("explored_nodes_number before opt %d is reached: %u / %u\n", 
                    *del_size, explored_node_number, del_explored_node_number);
                }
            }
#endif
        }
...
//                printf("bid %d del size %u ins size %u / %u\n", blockIdx.x, real_ins_size, *ins_size);
#ifdef DEBUG
