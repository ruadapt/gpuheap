...
__global__ void concurrentKernel(Heap<T> *heap, T *items, 
    ...
    if (blockIdx.x < gridDim.x / 2) {
        uint32_t size = 2 * batchSize / 3;
        uint32_t batchNeed = (arraySize + size - 1) / size;
        // insertion
        for (uint32_t i = blockIdx.x; i < batchNeed; i += gridDim.x / 2) {
            uint32_t insertSize = arraySize - i * size < size ? arraySize - i * size : size;
            heap->insertion(items + arraySize + i * size,
                            insertSize, 0);
            __syncthreads();
        }
    }
        ...
        int size = 0;
        uint32_t batchNeed = (arraySize + batchSize - 1) / batchSize;
        // deletion
...
__global__ void insertKernel(Heap<T> *heap, 
                             ...
                             uint32_t arraySize, 
                             uint32_t batchSize) {
    uint32_t size = batchSize / 3;
    uint32_t batchNeed = (arraySize + size - 1) / size;
    // insertion
    for (uint32_t i = blockIdx.x; i < batchNeed; i += gridDim.x) {
        uint32_t insertSize = arraySize - i * size < size ? arraySize - i * size : size;
        heap->insertion(items + i * size,
                        insertSize, 0);
        __syncthreads();
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // bitonic heap sort
    Heap<uint32_t> h_heap(batchNum, batchSize, UINT_MAX);
